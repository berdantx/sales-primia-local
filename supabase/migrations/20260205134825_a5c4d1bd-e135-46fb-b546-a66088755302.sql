-- Fix the get_ad_trend_data function with correct aggregation logic
  ...
  IF p_mode = 'ads' THEN
    ...
    IF p_group_by = 'week' THEN
      SELECT COALESCE(jsonb_agg(
        jsonb_build_object('date', date_key) || item_counts
        ORDER BY date_key
      ), '[]'::jsonb)
      INTO v_result
      FROM (
        ...
        FROM (
          ...
          GROUP BY DATE_TRUNC('week', created_at)::DATE, utm_content
        ) sub
        GROUP BY date_key
      ) grouped;
    ELSE
      ...
      FROM (
        ...
        FROM (
          ...
          WHERE created_at IS NOT NULL
            ...
            AND (p_start_date IS NULL OR created_at >= p_start_date)
            AND (p_end_date IS NULL OR created_at <= p_end_date)
          GROUP BY DATE_TRUNC('week', created_at)::DATE, utm_campaign
        ) sub
        GROUP BY date_key
      ) grouped;
    ELSE
      ...
      FROM (
        ...
        FROM (
          ...
          WHERE created_at IS NOT NULL
            ...
            AND (p_start_date IS NULL OR created_at >= p_start_date)
            AND (p_end_date IS NULL OR created_at <= p_end_date)
          GROUP BY DATE_TRUNC('week', created_at)::DATE, regexp_replace(regexp_replace(page_url, '^https?://', ''), '\?.*$', '')
        ) sub
        GROUP BY date_key
      ) grouped;
    ELSE
