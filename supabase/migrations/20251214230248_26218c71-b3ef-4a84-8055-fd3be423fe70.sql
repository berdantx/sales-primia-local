-- Fix Recuperador Inteligente grouping to use buyer_email + product + computed_value
-- This ensures different purchases (different computed_value) are kept separate
-- while installments of the same purchase (same computed_value) are grouped
...
BEGIN
  WITH deduplicated_transactions AS (
    ...
    -- Recuperador Inteligente: group by buyer/product/computed_value as single purchase
    SELECT 
    ...
    WHERE user_id = auth.uid()
      ...
      AND (p_start_date IS NULL OR purchase_date >= p_start_date)
      AND (p_end_date IS NULL OR purchase_date <= p_end_date)
    GROUP BY buyer_email, product, computed_value
  )
...
BEGIN
  WITH deduplicated_transactions AS (
    ...
    -- Recuperador Inteligente: group by buyer/product/computed_value, use first installment date
    SELECT 
      MAX(computed_value) as computed_value,
      MAX(currency) as currency,
      MIN(purchase_date) as purchase_date
