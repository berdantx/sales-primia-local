-- Corrigir função get_sales_by_date que está com erro de GROUP BY
...
BEGIN
  ...
  daily_totals AS (
    SELECT 
      TO_CHAR(purchase_date AT TIME ZONE 'America/Sao_Paulo', 'YYYY-MM-DD') as sale_date,
      currency,
      SUM(computed_value) as total
    FROM deduplicated_transactions
    GROUP BY TO_CHAR(purchase_date AT TIME ZONE 'America/Sao_Paulo', 'YYYY-MM-DD'), currency
  )
  SELECT json_object_agg(sale_date, currencies)
  FROM (
    SELECT 
      sale_date,
