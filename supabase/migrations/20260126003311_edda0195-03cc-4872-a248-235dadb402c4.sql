-- Corrige a função get_lead_stats para agrupar leads por dia usando timezone de Brasília
-- Antes: DATE(created_at) usava UTC, causando leads após 21h BRT aparecerem no dia seguinte
-- Depois: DATE(created_at AT TIME ZONE 'America/Sao_Paulo') agrupa corretamente
...
BEGIN
  SELECT json_build_object(
    ...
    'by_day', (
      ...
      FROM (
        -- CORRIGIDO: Usar timezone de Brasília para agrupar por dia
        SELECT DATE(created_at AT TIME ZONE 'America/Sao_Paulo') as day, COUNT(*) as cnt
        FROM leads
        WHERE client_id = p_client_id
          AND (p_start_date IS NULL OR created_at >= p_start_date)
          AND (p_end_date IS NULL OR created_at <= p_end_date)
