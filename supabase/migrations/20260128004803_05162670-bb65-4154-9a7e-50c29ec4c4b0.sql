...
-- Drop the older version of get_top_ads that has different parameter order
-- Keep only the optimized version with CTE that returns proper JSON structure

-- First, drop all versions
DROP FUNCTION IF EXISTS public.get_top_ads(uuid, timestamp with time zone, timestamp with time zone, text, integer);
DROP FUNCTION IF EXISTS public.get_top_ads(uuid, timestamp with time zone, timestamp with time zone, integer, text);

-- Recreate the optimized version with consistent parameter order
