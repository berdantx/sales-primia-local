...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'totalCount', COALESCE((SELECT max(lead_data.total_count) FROM (SELECT 1) x), 0)
  ) INTO result
  FROM (
    SELECT 
  ...
  -- Handle empty result
  IF result IS NULL OR result::text = 'null' THEN
    result := json_build_object('leads', '[]'::json, 'totalCount', 0);
  END IF;
