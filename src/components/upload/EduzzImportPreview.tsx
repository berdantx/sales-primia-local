...
import { EduzzTransaction, EduzzParseError } from '@/lib/parsers/eduzzParser';
...
interface EduzzImportPreviewProps {
...
export function EduzzImportPreview({ transactions, errors, duplicates, totalRows }: EduzzImportPreviewProps) {
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-orange-600">{formatCurrency(totalValue, 'BRL')}</p>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="font-medium text-warning">
                  {transactionsWithoutDate.length} transações sem data de venda
                </p>
                <p className="text-sm text-muted-foreground mt-1">
                  Essas transações serão importadas, mas não aparecerão em filtros de período.
                  Verifique se sua planilha contém a coluna "Data da venda".
                </p>
              </div>
            </div>
          </CardContent>
                  ...
                  <TableRow key={index}>
                    ...
                    <TableCell className="text-sm">
                      {transaction.sale_date ? (
                        format(transaction.sale_date, 'dd/MM/yyyy', { locale: ptBR })
                      ) : (
                    ...
                    <TableCell className="text-right font-medium">
                      {formatCurrency(transaction.sale_value, 'BRL')}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
